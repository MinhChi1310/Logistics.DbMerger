...
namespace Logistics.DbMerger
{
    class Program
    {
        ...
        static async Task RunDataSync(string sourceConn, string targetConn, int batchSize, bool dryRun)
        {
            ...
            // var missingTables = await schemaSync.GetMissingTablesAsync(); // Re-fetch to be safe

            // REVISED DATA SYNC LOGIC:
            // 1. Get All Source Tables.
            // 2. For each:
            //    - If exists in Target (Identity Match): It's a "Common Table". Use Tenant Filter or Skip (if no filter).
            //    - If missing: It's a "New Table" (Should have been created in Step 1, or we create now).
            
            // To keep it safe and consistent with previous "Add Missing" scope:
            // We should cache the "Mission List"? 
            // Or just check: "If table exists in Target, migrate data".
            // Tenant Filter is the safety guard for Common Tables.
            
            Console.WriteLine("Fetching Source Tables...");
            using var sConn = new SqlConnection(sourceConn);
            var sourceTables = await sConn.QueryAsync<string>("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE'");
            ...
            foreach(var table in sourceTables)
            {
                ...
                if (isNew)
                {
                     ...
                     // Check fuzzy
                     else if (table.EndsWith("s") && existingAdcTables.Contains(table.Substring(0, table.Length - 1), StringComparer.OrdinalIgnoreCase))
                     {
                         targetTable = existingAdcTables.First(t => t.Equals(table.Substring(0, table.Length - 1), StringComparison.OrdinalIgnoreCase));
                         isNew = false; 
                ...
                if (isNew)
                {
                    // If explicitly mapped but target doesn't exist, we treat it as new but using the mapped name?
                    // Usually if mapped, we expect target to exist or strictly want that name.
                    if (ExplicitTableMappings.ContainsKey(table))
                    {
                        targetTable = ExplicitTableMappings[table];
                    }

                    // New table. Did Step 1 create it? 
                    // If we are in Step 3, we expect it to exist if Step 1 ran.
                    // If it doesn't exist, we must create it or skip?
                    // Let's Create if missing (ensure).
                    if (!dryRun) await schemaSync.SyncTableAsync(table); // Note: SyncTableAsync currently uses source name. 
                    // If we want to create as target name, we need to update SyncTableAsync signature or assume strictly source name for new tables.
                    // For now, assuming new tables keep source name unless mapped (but if mapped, we likely want to map to existing).
                }
